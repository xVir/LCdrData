...
package enum BookmarkService {

    /// Returns bookmark data suitable for `UserDefaults`, or nil if creation fails.
    package nonisolated static func bookmarkData(for url: URL) -> Data? {
    ...
    package nonisolated static func url(fromBookmarkData data: Data) -> URL? {
        resolve(data).url
    }
    ...
    package nonisolated static func resolve(_ data: Data) -> (url: URL?, refreshedData: Data?) {
        var stale = false
