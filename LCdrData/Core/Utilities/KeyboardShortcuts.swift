...
package enum KeyboardShortcuts {
    ...
    package static let f2Key = KeyEquivalent(Character(UnicodeScalar(0xF705)!))

    /// F3 key equivalent for Quick Look
    package static let f3Key = KeyEquivalent(Character(UnicodeScalar(0xF706)!))

    /// F4 key equivalent for Edit / open
    package static let f4Key = KeyEquivalent(Character(UnicodeScalar(0xF707)!))

    /// F5 key equivalent for Copy
    package static let f5Key = KeyEquivalent(Character(UnicodeScalar(0xF708)!))

    /// F6 key equivalent for Move
    package static let f6Key = KeyEquivalent(Character(UnicodeScalar(0xF709)!))

    /// F7 key equivalent for Mkdir
    package static let f7Key = KeyEquivalent(Character(UnicodeScalar(0xF70A)!))

    /// F8 key equivalent for Delete
    package static let f8Key = KeyEquivalent(Character(UnicodeScalar(0xF70B)!))
}
