...
struct Cursor: Sendable, Equatable {
    ...
    init(focused: UUID? = nil, selected: Set<UUID> = []) {
    ...
    mutating func userDidSelect(_ newSelection: Set<UUID>) {
    ...
    mutating func selectAll(in listing: [FileItem]) {
    ...
    mutating func deselectAllKeepingFocus(in listing: [FileItem]) {
        if let focused {
            selected = [focused]
    ...
    mutating func focusFirst(in listing: [FileItem]) {
    ...
    /// non-parent rows exist. Empty listings are a no-op.
    mutating func focusLast(in listing: [FileItem]) {
